...
#### 3.2 기능 설명
1. `crawl_announcement.py`  
   - **기능**: 공지사항을 크롤링하는 모듈입니다. PLATO 플랫폼에서 새로운 공지사항을 가져오고, 해당 공지의 제목, 본문, 첨부파일을 저장합니다.
   - **주요 메서드**:
     - `get_anns_url()`: 특정 공지사항 페이지에서 공지 URL을 수집합니다.
     - `crawl_ann()`: 개별 공지의 상세 내용을 크롤링하여 제목, 본문, 첨부파일을 저장합니다.

2. `duplicate_checker.py`  
   - **기능**: 새로운 공지사항의 제목이 최근 공지와 중복되는지 여부를 확인하는 모듈입니다. 공지의 유사도를 계산하고 GPT API를 이용하여 중복 여부를 판단합니다.
   - **주요 메서드**:
     - `is_recent_title_duplicate()`: 저장된 공지와 새로운 공지의 제목을 비교하여 중복 여부를 판단합니다.
     - `save_title()`: 새로운 공지 제목을 텍스트 파일에 저장합니다.

3. `gpt_client.py`  
   - **기능**: OpenAI GPT API를 이용하여 공지사항을 분류하고 중복 여부를 판단하는 기능을 제공합니다.
   - **주요 메서드**:
     - `answer_gpt()`: 공지사항의 내용을 GPT에게 보내 카테고리를 분류받습니다.
     - `check_title_similarity()`: GPT를 이용해 새로운 공지 제목이 기존 제목들과 중복되는지 여부를 확인합니다.

4. `main.py`  
   - **기능**: 프로젝트의 메인 실행 파일로, 공지사항을 크롤링하고 중복 여부를 체크하며, 분류한 후 공지사항을 PLATO에 업로드하는 전체적인 흐름을 관리합니다.

5. `page_url_manager.py`  
   - **기능**: PLATO 플랫폼의 페이지 URL을 관리하는 모듈입니다. 크롤링을 진행할 URL을 지정하고 공지사항 URL을 저장합니다.

6. `selenium_service.py`  
   - **기능**: Selenium을 이용하여 공지사항을 PLATO 플랫폼에 자동으로 게시하는 기능을 제공합니다.
   - **주요 메서드**:
     - `write_notices()`: 크롤링된 공지사항을 PLATO의 공지사항 게시판에 자동으로 작성합니다.
     - `remove_stars_and_uncheck_notices()`: 기존 공지사항에서 중요 표시된 항목의 별을 제거하고, 공지 상태를 해제하는 기능을 제공합니다.
     
7. `pages.csv`  
   - **기능**: PLATO에서 공지사항을 수집하는데 필요한 페이지 정보가 저장된 CSV 파일입니다. 이 파일은 공지사항이 올라오는 URL들을 관리합니다.

8. `titles.txt`  
   - **기능**: 이미 크롤링된 공지사항 제목들을 저장하는 텍스트 파일입니다. 중복 여부를 확인하기 위해 사용됩니다.
